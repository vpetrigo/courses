...
class NetworkQueue {
 public:
  explicit NetworkQueue(const std::size_t queue_size)
      : queue_{std::make_unique<T[]>(queue_size)},
        queue_indexes_{0, 0},
        queue_size_{0},
  ...
  bool Empty() const { return (queue_size_ == 0); }
  bool Full() const { return (queue_size_ == queue_capacity_); }
  ...
  const T& Top() const { return queue_[queue_indexes_.first]; }
  T& Top() { return queue_[queue_indexes_.first]; };
  const T& Back() const
  {
    return queue_[(queue_indexes_.second == 0) ? queue_capacity_ - 1
                                               : queue_indexes_.second - 1];
  }
