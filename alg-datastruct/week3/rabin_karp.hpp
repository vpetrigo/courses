...
template <typename T> T pow_fast_modulo(T base, T exp, T modulo)
{
    std::size_t result = 1;
                              ...
                              std::size_t modulo)
{
    ...
    while (first != last) {
        hash_val = (hash_val + static_cast<std::size_t>(*first++) *
                                   pow_fast_modulo(coeff, --max_power, modulo) %
                                   modulo) %
                   modulo;
    }
...
class RabinKarp {
  public:
    ...
    explicit RabinKarp(const std::string &pattern)
        : pattern_{pattern}, pattern_hash_{StrPolynomialHash(
                                 std::rbegin(pattern_), std::rend(pattern_),
                                 coeff, prime)},
          pattern_length_{pattern_.size()}, x_p_{pow_fast_modulo(
                                                coeff, pattern_length_ - 1,
                                                prime)}
    {
    }
    ...
    std::vector<std::size_t> GetPatternOccurence(const std::string &text)
    {
        ...
        *window_hash_it++ =
        ...
        while (window_end != text_end + 1) {
            ...
            new_hash =
                (*std::prev(window_hash_it) + prime -
                 x_p_ * static_cast<std::size_t>(*std::prev(window_begin)) %
                     prime) %
                prime;
            new_hash =
                (new_hash * coeff % prime + *std::prev(window_end)) % prime;
        ...
        for (std::size_t i = window_hash.size() - 1; i < window_hash.size();
             --i) {
